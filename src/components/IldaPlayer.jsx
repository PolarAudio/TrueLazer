...
const IldaPlayer = ({ frame, effects, showBeamEffect, beamAlpha, fadeAlpha, previewScanRate, beamRenderMode, intensity, syncSettings = {}, bpm = 120, clipDuration = 1, progress = 0, previewTime = 0, onToggleBeamEffect, onCycleDisplayMode }) => {
  ...
  useEffect(() => {
    ...
    worker.postMessage({
      ...
      payload: {
        ...
        canvas: offscreen,
        type: 'single',
        data: { showBeamEffect, beamAlpha, fadeAlpha, previewScanRate, beamRenderMode, intensity, syncSettings, bpm, clipDuration, progress, previewTime }
      }
    }, [offscreen]);
  ...
  useEffect(() => {
    ...
    worker.postMessage({
      ...
      payload: {
        id: canvasId.current,
        data: { ildaFrames: framesToSend, effects: effects, showBeamEffect, beamAlpha, fadeAlpha, previewScanRate, beamRenderMode, intensity, syncSettings, bpm, clipDuration, progress, previewTime }
      }
    });
  }, [worker, frame, effects, showBeamEffect, beamAlpha, fadeAlpha, previewScanRate, beamRenderMode, intensity, syncSettings, bpm, clipDuration, progress, previewTime]);
