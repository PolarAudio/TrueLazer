...
## Core Features
- **Professional Integration:** High priority is placed on robust MIDI, OSC, and DMX/Artnet mapping. Includes a **Hybrid DMX System** with fixed personalities for rapid console setup (Master + 5 Layers) and custom modular patching for any effect parameter. High-frequency control traffic (MIDI, Art-Net, Keyboard) is handled via a **Global 60Hz Throttled Dispatch** (16.6ms) with leading and trailing execution, ensuring perfectly stable 60fps UI performance and immediate feedback even under extreme control density. Features a real-time **16x32 DMX Monitor** for channel verification and network interface binding for dedicated control networks.
