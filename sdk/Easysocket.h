...
namespace SocketLib
{
    ...
    protected:
        ...
        // ====================================================================
        Socket( SOCKET p_socket = INVALID_SOCKET );


        SOCKET m_sock;                    // this is the underlying representation
	...
	{
	public:
		UDPSocket( port p_port, ipaddress p_addr = 0 );
		~UDPSocket ();
		int UDPSendTo( ipaddress p_addr, port p_port, const char* p_buffer, int p_size );
		int UDPReceive( char* p_buffer, int p_size );
		int UDPReceiveFrom( char* p_buffer, int p_size, char* remote_ip );
		void set_broadcast(bool broadcast);
		void set_time_out(int timeout);
		int get_interfaces(std::vector<std::string> &interfaces);
		void get_ip_and_mask(const char* iface, unsigned long &ip, unsigned long &sub_mask);
	protected:
    ...
    public:
        ...
        // ====================================================================
        DataSocket( SOCKET p_socket = INVALID_SOCKET );
