...
bool DameiSDK::Init(char ip[4])
{
	...
	try
	{
		udpSocket = new SocketLib::UDPSocket(DAC_LIST_PORT + 10);
		inited = true;
	}
...
bool DameiSDK::SendPointsToShow(int showIndex, frame_buffer &frameBuffer)
{
	...
	if(showIndex < showList.count && showList.udpPort[showIndex] > UDP_DAC_EXTERNAL_PORT_BEGIN){
		...
		int rCount = udpSocket->UDPSendTo(ipAddress, showList.udpPort[showIndex], (const char*)&frameBuffer, sizeof(frameBuffer));
